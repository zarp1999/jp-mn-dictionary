...
void user_button_init(void)
{
  	...
  	button_attach(&button1, BTN_LONG_PRESS_START, on_boot_longpress_press);
  	button_attach(&button1, BTN_PRESS_UP, on_boot_pressup_press);

  	button_init(&button2, read_button_GPIO, button2_active , button2_id);
  	button_attach(&button2, BTN_SINGLE_CLICK, on_pwr_single_click);
  	button_attach(&button2, BTN_DOUBLE_CLICK, on_pwr_double_press);
  	button_attach(&button2, BTN_LONG_PRESS_START, on_pwr_longpress_press);
  	button_attach(&button2, BTN_PRESS_UP, on_pwr_pressup_press);
